...
const TourGuideAnalyticsPage = () => {
    ...
    const calcRevenue = async () => {
        setRevenueData([]);
        try {
            const response = await calcTourguideRevenue({
                id: id,
            ...
            // set itineraries to array of itinerary titles
            setItineraries(response.data.map((data) => data.itineraryTitle));
            if (selectedItineraries.length === 0) {
                setSelectedItineraries(response.data.map((data) => data.itineraryTitle));
            }
        } catch (error) {
            console.log(error);
        }
    ...
    useEffect(() => {
        ...
        getRevenue();
        const tableData = revenueData.filter((data) => selectedItineraries.includes(data.itineraryTitle));
        setTableData(tableData);
        setPieChartData(tableData.map((data) => { return { id: data.itineraryTitle, value: data.totalRevenue, label: data.itineraryTitle } }));
    }, [startDate, endDate]);
    ...
    useEffect(() => {
        ...
        setTableData(tableData);
        setPieChartData(tableData.map((data) => { return { id: data.itineraryTitle, value: data.totalRevenue, label: data.itineraryTitle } }));
    }, [selectedItineraries, revenueData]);
    ...
    const rows = tableData.map((data) => {
        return createData(data.itineraryTitle, data.itineraryPrice, data.bookingCount, data.totalRevenue);
    });
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <FormControl sx={{ m: 1, width: 250 }}>
                    <InputLabel id="demo-multiple-name-label">Itineraries</InputLabel>
                    <Select
                        ...
                        input={<OutlinedInput label="Itineraries" />}
                    >
                        {/* "All" Option */}
                        <MenuItem
                            key="all"
                        ...
                        {itineraries.map((name) => (
                            <MenuItem
                                key={name}
                                value={name}
                                style={{
                                    color:
                                        selectedItineraries.indexOf(name) === -1
                                            ? 'lightgrey'
                                            : 'black',
                                }}
                            >
...
export default TourGuideAnalyticsPage;